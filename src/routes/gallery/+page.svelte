<script lang="ts">
	...
	function loadImages(category: string) {
		return imagesByCategory[category as keyof typeof imagesByCategory] || [];
	}

	function selectCategory(category: (typeof categories)[0]) {
		selectedCategory = category;
		images = loadImages(category.id);
	}
	...
	onMount(() => {
		images = loadImages(selectedCategory.id);
	});
</script>
...
<div class="container mx-auto px-4 py-8">
	...
	<div class="mb-8 flex flex-wrap gap-4">
		{#each categories as category}
			<button
				class="rounded-full px-6 py-2 text-sm font-medium transition-colors {selectedCategory.id ===
				category.id
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
		{#each images as image}
			...
			>
				...
				/>
				<div
					class="absolute inset-0 bg-gradient-to-t from-gray-900/90 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
				>
